...
symbol_count = { #define the symbols in a dict
    "A": 2,
    "B": 4,
    "C": 6,
    "D": 8
...
symbol_value = { #define value of the symbols
    "A": 5,
    "B": 4,
    "C": 3,
    "D": 2
...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    for _ in range(cols):
        column = []
        current_symbols = all_symbols[:] #slice operator so original is preserved
        for _ in range(rows):
...
def spin(balance): # executes a single game
    ...
    slots = get_slot_machine_spin(NUM_ROWS, NUM_ROWS, symbol_count)
